...
export default function Button({ className, children, color, to, type, ...rest }: ButtonProps) {
  ...
  if (to)
    return (
      <Link
        className={`${twMerge(
          'w-fit flex gap-4 px-4 py-1 relative rounded-md items-center justify-center text-base  font-sans font-semibold drop-shadow-sm',
          handleColors(color),
          className,
        )} `}
        to={to}>
